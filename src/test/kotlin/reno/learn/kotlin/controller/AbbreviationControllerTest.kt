...
internal class AbbreviationControllerTest : BehaviorSpec({
    ...
    Given("a controller") {
        var abbreviationService = mockk<AbbreviationService>()
        var abbreviationController = AbbreviationController(abbreviationService)

        When("the service is being called for meanings and has one result") {
            every { abbreviationService.retrieveMeaning(resultShortForm1) } returns
                    listOf(resultMeaning1)
            Then("it returns a list of possible meanings") {
                var result = abbreviationController.retrieveMeaning(testCaseShortForm)
